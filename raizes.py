...
class Function:
    ...
    def newton_raphson(self):
        ...
        for iter in range(niter):
            ...
            f = [self.evaluate_f1(x), self.evaluate_f2(x), self.evaluate_f3(x)]
            delta_x =  self.mult_matrix_vector(self.inverse3(j), f)
            for i in range(3): x[i] = -1*(x[i] + delta_x[i])
...
x = Function(icod=1, tolm=0.00001, t1=0, t2=3)
print(x.mult_matrix_vector(m,v))